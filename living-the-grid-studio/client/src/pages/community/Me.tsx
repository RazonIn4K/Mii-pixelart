    ...
    <CommunityShell>
      <RequireAuth>
        <div className="container py-12 sm:py-16">
          ...
          {user ? <div className="mt-10 flex flex-col items-start gap-5 rounded-3xl border-2 border-[var(--island-ink)] bg-white p-6 shadow-[6px_6px_0_var(--island-ink)] sm:flex-row sm:items-center"><IslandAvatar seed={user.avatarSeed} className="h-20 w-20" /><div className="flex-1"><h2 className="text-xl font-black">{user.displayName}</h2><p className="text-sm text-muted-foreground">{user.username ? `@${user.username}` : "Your public identity is not finished yet."}</p></div>{!user.username || user.termsAccepted !== true ? <Button asChild><Link href="/me/setup">{user.username ? "Review current terms" : "Finish profile"}</Link></Button> : null}</div> : null}
        </div>
      </RequireAuth>
    </CommunityShell>
  );

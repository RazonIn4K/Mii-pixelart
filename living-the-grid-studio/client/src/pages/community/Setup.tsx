          ...
          <div className="mx-auto grid max-w-4xl gap-8 lg:grid-cols-[280px_1fr] lg:items-start">
            ...
            <form onSubmit={submit} className="community-detail-panel">
              ...
              <div className="mt-8 space-y-5">
                  ...
                  <div id="username-suggestions" aria-live="polite">
                    {usernameSuggestions.length ? (
                      <div className="rounded-xl border border-[var(--island-blue)]/40 bg-[var(--island-blue-soft)] p-3">
                        <p className="text-xs font-bold text-[var(--island-ink)]">That name is taken. Try one of these:</p>
                        <div className="mt-2 flex flex-wrap gap-2">
                          {usernameSuggestions.map((suggestion) => (
                            <button key={suggestion} type="button" onClick={() => { setUsername(suggestion); setUsernameSuggestions([]); }} className="rounded-full border border-[var(--island-ink)]/20 bg-white px-3 py-1 font-mono text-xs font-bold hover:border-[var(--island-ink)]">
                              @{suggestion}
                            </button>
                          ))}
                        </div>
                ...
                <div className="space-y-2"><Label htmlFor="profile-display-name">Display name</Label><Input id="profile-display-name" value={displayName} onChange={(event) => setDisplayName(event.target.value.slice(0, 50))} maxLength={50} required /></div>

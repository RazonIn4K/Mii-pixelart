...
export default function CreationDetailPage() {
  ...
  const loadComments = useCallback(
    async (creationId: string, requestId: number, nextCursor?: string) => {
      const isCurrent = () => loadRequestRef.current === requestId;
      if (nextCursor) setCommentsLoadingMore(true);
      try {
        const commentResult = await communityApi<CommunityComment[]>(
          `/api/creations/${creationId}/comments${nextCursor ? `?cursor=${encodeURIComponent(nextCursor)}` : ""}`,
        );
        if (!isCurrent()) return;
        setComments((current) => {
          if (!nextCursor) return commentResult.data;
          const unique = new Map(
            [...current, ...commentResult.data].map((comment) => [
              comment.id,
              comment,
            ]),
          );
  ...
  const load = useCallback(async () => {
    ...
    try {
      const result = await communityApi<CreationDetail>(
        `/api/public/creations/${encodeURIComponent(slug)}`,
      );
      if (!isCurrent()) return;
  ...
  const toggleLike = async () => {
    ...
    setCreation({
      ...creation,
      isLiked: liked,
      likeCount: Math.max(0, creation.likeCount + (liked ? 1 : -1)),
    });
    try {
  ...
  const addComment = async (event: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const result = await communityApi<CommunityComment>(
        `/api/creations/${creation.id}/comments`,
        {
          method: "POST",
  ...
  return (
    <CommunityShell>
      <div className="container py-10 sm:py-16">
        {loading ? (
          <CommunityLoading label="Opening this creation…" />
        ) : error || !creation ? (
          <CommunityError
            message={error ?? "Creation not found"}
            retry={() => void load()}
          />
        ) : (
          <>
            <div className="grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] lg:items-start">
              <div className="space-y-3">
                <CreationShowcaseGallery
                  ...
                  fallbackImageUrl={
                    creation.primaryImageUrl ?? creation.previewUrl
                  }
              ...
              <aside className="community-detail-panel">
                <div className="flex flex-wrap items-center gap-2">
                  {creation.tags.map((tag) => (
                    <Link
                      ...
                      className="rounded-full bg-[var(--island-blue-soft)] px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em]"
                    >
                      {tag}
                    </Link>
                  ))}
                </div>
                <h1 className="mt-5 text-4xl font-black tracking-[-0.05em] text-[var(--island-ink)]">
                  {creation.title}
                </h1>
                {creation.description ? (
                  <p className="mt-4 whitespace-pre-line text-sm font-medium leading-6 text-[var(--island-ink)]/65">
                    {creation.description}
                  </p>
                ) : null}
                <Link
                ...
                >
                  ...
                  <span>
                    <span className="block text-sm font-black">
                      {creation.owner.displayName}
                    </span>
                    <span className="block text-xs text-[var(--island-ink)]/50">
                      @{creation.owner.username}
                    </span>
                  </span>
                </Link>
                <p className="mt-4 text-xs font-bold text-[var(--island-ink)]/45">
                  Published {formatCommunityDate(creation.publishedAt)}
                </p>
                <div className="mt-6 grid grid-cols-2 gap-2">
                  <Button
                    ...
                    onClick={toggleLike}
                  >
                    <Heart className={creation.isLiked ? "fill-current" : ""} />{" "}
                    {formatCount(creation.likeCount)}
                  </Button>
                  {creation.downloadEnabled ? (
                    <Button asChild variant="outline">
                      <a
                        href={`/api/creations/${creation.id}/media/project`}
                        download
                      >
                        <Download /> Studio project (.json)
                      </a>
                    </Button>
                  ) : null}
                  {creation.canEdit ? (
                    <Button asChild>
                      <Link href={`/studio?cloud=${creation.id}`}>
                        Edit in Studio
                      </Link>
                    </Button>
                  ) : null}
                ...
                <div className="mt-3 rounded-2xl bg-white p-3">
                  <p className="mb-2 text-xs font-black uppercase tracking-[0.12em] text-[var(--island-ink)]/50">
                    Share this creation
                  </p>
                ...
                <div className="mt-4 flex justify-end">
                  <ReportDialog targetType="creation" targetId={creation.id} />
                </div>
              </aside>
            </div>
            ...
            >
              <div className="flex items-center justify-between gap-4">
                ...
                >
                  ...
                  <span className="text-sm text-[var(--island-ink)]/45">
              ...
              {!creation.commentsEnabled || creation.commentsLocked ? (
                <p className="mt-5 rounded-2xl bg-white p-5 text-sm text-[var(--island-ink)]/60">
                  {creation.commentsLocked
                    ? "A moderator locked new comments. Existing discussion remains readable."
                    : "The creator turned off new comments. Existing discussion remains readable."}
                </p>
              ) : user ? (
                <form
                  onSubmit={addComment}
                  className="mt-5 rounded-2xl border-2 border-[var(--island-ink)] bg-white p-4 shadow-[4px_4px_0_var(--island-ink)]"
                >
                  <label htmlFor="new-comment" className="text-sm font-black">
                  ...
                  <Textarea
                    ...
                    rows={3}
                    className="mt-2"
                    placeholder="Be kind and keep it constructive."
                  />
                ...
                <p className="mt-5 rounded-2xl bg-white p-5 text-sm text-[var(--island-ink)]/60">
                  Sign in to comment. Reading stays open to everyone.
                </p>
              )}
                ...
                {comments.length ? (
                  comments.map((comment) => {
                    ...
                    return (
                      <article
                        key={comment.id}
                        className="rounded-2xl border border-[var(--island-ink)]/10 bg-white p-4"
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex flex-wrap items-baseline gap-x-2">
                              ...
                              <span className="text-xs text-[var(--island-ink)]/45">
                            ...
                            {editing ? (
                              ...
                              <p className="mt-2 whitespace-pre-line text-sm leading-6 text-[var(--island-ink)]/70">
                                {comment.body}
                              </p>
                            )}
                          </div>
                  ...
                  <p className="py-8 text-center text-sm text-[var(--island-ink)]/50">
                    No comments yet.
                  </p>
                )}
                {commentsError ? (
                  <div
                    className="rounded-2xl border border-destructive/30 bg-red-50 p-4 text-sm text-red-950"
                    role="alert"
                  >
                    <p>{commentsError}</p>
                    <Button
                      ...
                      onClick={() =>
                        void loadComments(creation.id, loadRequestRef.current)
                      }
                    >
                      Retry comments
                    </Button>

...
interface ExportProfileImageRow {
  byte_size: number;
  content_type: string;
  created_at: number;
  focus_x: number;
...
async function setupProfile(context: WorkerRequestContext): Promise<Response> {
  ...
  try {
    ...
    if ((result.meta.changes ?? 0) !== 1) {
      throw new HttpError(
        409,
        "profile_already_configured",
        "This profile is already configured.",
      );
    }
  } catch (error) {
    if (error instanceof HttpError) throw error;
    if (error instanceof Error && /unique constraint/iu.test(error.message)) {
      const suggestions = await availableUsernameSuggestions(
        context.env,
        input.username,
      );
      throw new HttpError(
...
async function listSessions(context: WorkerRequestContext): Promise<Response> {
  ...
  const rows = await context.env.DB.prepare(
    ...
    .bind(session.user.id, Date.now(), COMMUNITY_LIMITS.activeSessionsPerUser)
    .all<{
      created_at: number;
      expires_at: number;
      id: string;
  ...
  context.executionCtx.waitUntil(
    (async () => {
      try {
        ...
        await writer.write(
          encoder.encode(
            `${JSON.stringify({ type: "account", data: account })}\n`,
          ),
        );
        await writeProfileImageExport(
          context,
          session.user.id,
          writer,
          encoder,
        ...
        for (const query of [
          ...
          },
          {
            ...
            table: "likes",
            predicate: "user_id",
            idColumn: "creation_id",
          },
          {
            type: "following",
            table: "follows",
            predicate: "follower_user_id",
            idColumn: "followed_user_id",
          },
          {
            type: "follower",
            table: "follows",
            predicate: "followed_user_id",
            idColumn: "follower_user_id",
          },
          {
            type: "report",
            table: "reports",
            predicate: "reporter_user_id",
            idColumn: "id",
          },
          {
          ...
          {
            type: "profile_image_upload_attempt",
            table: "profile_image_upload_attempts",
            predicate: "user_id",
            idColumn: "id",
          },
...
async function writeProfileImageExport(
  ...
  const image = await context.env.DB.prepare(
    `SELECT pi.id, pi.focus_x, pi.focus_y, pi.source_width, pi.source_height,
      pi.created_at, pi.updated_at, pio.object_key, pio.content_type,
      pio.byte_size, pio.sha256
     FROM users u
     JOIN profile_images pi ON pi.id = u.avatar_image_id AND pi.status = 'ready'
     JOIN profile_image_objects pio ON pio.image_id = pi.id AND pio.status = 'ready'
     WHERE u.id = ? LIMIT 1`,
  )
    .bind(userId)
    .first<ExportProfileImageRow>();
  if (!image) return;
  ...
  const object = await context.env.PROJECTS.get(image.object_key);
  if (!object) return;
  ...
  try {
    while (true) {
      ...
      if (done) break;
      await writeNdjson(writer, encoder, {
        type: "profile_image_object_chunk",
        imageId: image.id,
        sequence,
...
async function writePagedRows(
  ...
  while (true) {
    ...
    const rows: D1Result<ExportPageRow> = await db
      .prepare(
...
async function requestDeletion(
  ...
  response.headers.append(
    "Set-Cookie",
    cookie(
      new URL(context.env.PUBLIC_SITE_URL).protocol === "https:"
        ? "__Host-tomodachi.sid"
        : "tomodachi.sid",
      "",
      {
        maxAge: 0,
...
async function cancelDeletion(
  ...
  const result = await context.env.DB.prepare(
    ...
    .bind(now, session.user.id, now)
    .run();
  if ((result.meta.changes ?? 0) !== 1) {
    throw new HttpError(
      409,
      "deletion_not_cancelable",
      "Account deletion cannot be canceled.",
    );
  }
  return success(
    context.requestId,
...
async function getPublicProfileCreations(
  context: WorkerRequestContext,
): Promise<Response> {
  const username = normalizeUsername(context.params.username);
  const user = await publicProfile(context.env, username);
...
async function followUser(context: WorkerRequestContext): Promise<Response> {
  ...
  const target = await publicProfile(
    context.env,
    normalizeUsername(context.params.username),
  );
  if (!target)
    throw new HttpError(404, "profile_not_found", "Profile was not found.");
  ...
  const result = await context.env.DB.prepare(
    ...
    .bind(session.user.id, target.id, Date.now())
    .run();
  return success(context.requestId, {
    following: true,
...
async function unfollowUser(context: WorkerRequestContext): Promise<Response> {
  ...
  const result = await context.env.DB.prepare(
    ...
    .bind(session.user.id, target.id)
    .run();
  return success(context.requestId, {
    following: false,
    changed: (result.meta.changes ?? 0) === 1,
...
async function listConnections(
  ...
  const rows = await context.env.DB.prepare(
    `SELECT u.id, u.username, u.display_name, u.avatar_seed,
      u.avatar_image_id, f.created_at AS followed_at
...
export async function getAccount(
  env: Env,
  userId: string,
): Promise<ReturnType<typeof accountToApi> | null> {
  const row = await env.DB.prepare(
    `SELECT u.id, u.username, u.display_name, u.bio, u.role, u.status,
     u.avatar_seed, u.avatar_image_id, u.terms_version, u.terms_accepted_at, u.deletion_due_at,
...
function accountToApi(row: AccountRow, requiredTermsVersion: string) {
  return {
    ...
    termsAccepted:
      ...
      row.terms_version === requiredTermsVersion,
...
async function publicProfile(
  ...
  return env.DB.prepare(
    `SELECT u.id, u.username, u.display_name, u.bio, u.avatar_seed,
      u.avatar_image_id, u.created_at,
...
async function availableUsernameSuggestions(
  ...
  const rows = await env.DB.prepare(
    ...
    .bind(requested, COMMUNITY_LIMITS.usernameMaximum)
    .all<{ username: string }>();
  return rows.results.map((row) => row.username);
}

function parseTimestampCursor(
  value: string | null,
